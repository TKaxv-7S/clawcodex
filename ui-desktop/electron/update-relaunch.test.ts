 ...
 *      internals; CLAWCODEX_CONFIG_DIR / CLAWCODEX_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'clawcodex')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_ClawCodex12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/clawcodex/clawcodex', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/ClawCodex/clawcodex', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.clawcodex/clawcodex/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'clawcodex'), '', 'linux'), null)
})
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'clawcodex://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'clawcodex://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves CLAWCODEX_CONFIG_DIR + CLAWCODEX_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    CLAWCODEX_CONFIG_DIR: '/home/u/.clawcodex',
    CLAWCODEX_DESKTOP_REMOTE_URL: 'http://box:9119',
    CLAWCODEX_DESKTOP_REMOTE_TOKEN: 'secret',
    CLAWCODEX_DESKTOP_CLAWCODEX_ROOT: '/home/u/dev/clawcodex',
    CLAWCODEX_DESKTOP_APP_NAME: 'ClawCodexSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  ...
  assert.match(script, /export CLAWCODEX_CONFIG_DIR='\/home\/u\/\.clawcodex'/)
  assert.match(script, /export CLAWCODEX_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/ClawCodex' 'clawcodex:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `clawcodex-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/ClawCodex/ClawCodex',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `clawcodex-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/ClawCodex\/ClawCodex'\n/)
})
