...
import {
  ...
  resolveVenvClawCodexCommand
} from './windows-clawcodex-path'
import {
...
const USER_DATA_OVERRIDE = process.env.CLAWCODEX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLAWCODEX_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CLAWCODEX_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CLAWCODEX_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[clawcodex] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[clawcodex] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. CLAWCODEX_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[clawcodex] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[clawcodex] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `clawcodex desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose ClawCodex sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[clawcodex] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[clawcodex] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[clawcodex] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[clawcodex] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[clawcodex] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('clawcodex:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized ClawCodex burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clawcodex] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[clawcodex] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[clawcodex] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clawcodex] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLAWCODEX_CONFIG_DIR — the user-facing root for everything ClawCodex-related. Mirrors
// scripts/install.ps1's $ClawCodexHome and scripts/install.sh's $CLAWCODEX_CONFIG_DIR.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clawcodex (matches install.ps1)
//   macOS / Linux: ~/.clawcodex (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clawcodex directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clawcodex yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLAWCODEX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLAWCODEX_CONFIG_DIR beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clawcodex / %LOCALAPPDATA%\clawcodex.
function resolveClawCodexHome() {
  if (process.env.CLAWCODEX_CONFIG_DIR) {
    return normalizeClawCodexHomeRoot(process.env.CLAWCODEX_CONFIG_DIR)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'clawcodex-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CLAWCODEX_CONFIG_DIR set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\clawcodex and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CLAWCODEX_CONFIG_DIR')

    if (fromRegistry) {
      return normalizeClawCodexHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'clawcodex')
    const legacy = path.join(app.getPath('home'), '.clawcodex')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.clawcodex setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.clawcodex')
}

const CLAWCODEX_CONFIG_DIR = resolveClawCodexHome()

function pathWithClawCodexManagedNode(...entries) {
  const managed = clawcodexManagedNodePathEntries(CLAWCODEX_CONFIG_DIR).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CLAWCODEX_ROOT — the canonical mutable ClawCodex install. Same path
...
// boots, going straight to `resolveClawCodexBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLAWCODEX_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLAWCODEX_ROOT, '.clawcodex-bootstrap-complete')
...
// active-profile.json records which ClawCodex profile the desktop launches its
// local backend as. When set, startClawCodex() passes `clawcodex --profile <name>
// dashboard …`, which deterministically pins CLAWCODEX_CONFIG_DIR (see
// _apply_profile_override in clawcodex_cli/main.py) and bypasses the sticky
// ~/.clawcodex/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors clawcodex_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// clawcodexDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLAWCODEX_CONFIG_DIR/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clawcodex_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CLAWCODEX_CONFIG_DIR, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (clawcodex_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CLAWCODEX_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.CLAWCODEX_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.CLAWCODEX_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CLAWCODEX_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CLAWCODEX_DESKTOP_APP_NAME || 'ClawCodex'
...
// 'clawcodex:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.clawcodex.desktop) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.clawcodex.desktop')
}

// Seed the native About panel with the live ClawCodex version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `clawcodex update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveClawCodexVersion(),
  copyright: 'Copyright © 2026 ClawCodex contributors'
})
...
const MEDIA_PROTOCOL = 'clawcodex-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(clawcodex-media://…)`.
...
// startClawCodex(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLAWCODEX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLAWCODEX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startClawCodex() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning clawcodex serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start ClawCodex backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[clawcodex] ${line}`)
  clawcodexLog.push(...lines)

  if (clawcodexLog.length > 300) {
    clawcodexLog.splice(0, clawcodexLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clawcodex-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('clawcodex:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clawcodex:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('clawcodex:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_CLAWCODEX_ROOT
  })
}
...
// The Tauri updater writes CLAWCODEX_CONFIG_DIR/.clawcodex-update-in-progress for the whole
...
// (`force_kill_other_clawcodex`, taskkill /IM clawcodex.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(CLAWCODEX_CONFIG_DIR)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — ClawCodex will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `clawcodex` — must not shadow `clawcodex.cmd`/`clawcodex.exe`.
...
function unwrapWindowsVenvClawCodexCommand(command, backendArgs) {
  return resolveVenvClawCodexCommand(command, backendArgs, {
...
function backendSupportsServe(backend) {
  ...
  if (supported === null) {
    try {
      ...
      // `clawcodex --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CLAWCODEX_CONFIG_DIR, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveClawCodexBackend step 4); execFileSync of a .cmd without
...
function findPythonForRoot(root) {
  const override = process.env.CLAWCODEX_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several ClawCodex deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the clawcodex-import
...
// findGitBash — locate bash.exe on Windows. Resolves CLAWCODEX_GIT_BASH_PATH
...
// normal CLAWCODEX_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\clawcodex\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clawcodex', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clawcodex', 'git', 'bin', 'git.exe'))
  }
...
function recentClawCodexLog() {
  return clawcodexLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clawcodex root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLAWCODEX_ROOT.
// CLAWCODEX_DESKTOP_CLAWCODEX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLAWCODEX_DESKTOP_CLAWCODEX_ROOT && path.resolve(process.env.CLAWCODEX_DESKTOP_CLAWCODEX_ROOT),
    !IS_PACKAGED && isClawCodexSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isClawCodexSourceRoot(ACTIVE_CLAWCODEX_ROOT) ? ACTIVE_CLAWCODEX_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLAWCODEX_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clawcodex:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `clawcodex update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(CLAWCODEX_CONFIG_DIR, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `clawcodex update` can write
// fresh entry points. On Windows this is the file the running backend
// `clawcodex.exe` holds open; on POSIX it's never mandatory-locked.
function venvClawCodexShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clawcodex.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clawcodex')
}
...
// only signals the direct child, so on Windows a backend `clawcodex.exe` that
// spawned its own grandchildren (a `clawcodex` REPL, a pty terminal session, the
...
// `clawcodexProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clawcodex.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a clawcodex
// REPL, a pty terminal, the gateway) keep `clawcodex.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const clawcodexProcess = backendConnectionState.getProcess()

  if (clawcodexProcess && Number.isInteger(clawcodexProcess.pid)) {
    pids.push(clawcodexProcess.pid)
  }
  ...
  if (clawcodexProcess && !clawcodexProcess.killed) {
    try {
      clawcodexProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvClawCodexShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentClawCodexProcess = backendConnectionState.getProcess()

    if (currentClawCodexProcess && Number.isInteger(currentClawCodexProcess.pid)) {
      stragglers.push(currentClawCodexProcess.pid)
    }
...
// `clawcodex update`). Instead we spawn the staged ClawCodex-Setup binary with
// --update and quit, so it can run `clawcodex update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged clawcodex-setup or not — the resolver
      ...
      // `clawcodex update` (backend) + `clawcodex desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // `clawcodex desktop`, never the Tauri installer that self-copies
      // clawcodex-setup.exe into CLAWCODEX_CONFIG_DIR). They DO have a working `clawcodex`
      ...
      // checkout they're on — bare `clawcodex update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `clawcodex update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `clawcodex update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, clawcodexRoot: updateRoot }
    }

    const handoffConflict = updateHandoffConflict(CLAWCODEX_CONFIG_DIR)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating ClawCodex — this window will close and the updater will open. Don’t reopen ClawCodex yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(CLAWCODEX_CONFIG_DIR, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // clawcodex.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running clawcodex, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the ClawCodex install open ' +
        '(a second ClawCodex window or a terminal running clawcodex?). Close it and retry.'

      emitUpdateProgress({ stage: 'error', message, percent: null })
      startClawCodex().catch(() => {})

      return { ok: false, error: message }
    }

    // Preflight: after releasing our own backends, check for remaining
    // ClawCodex processes running from this venv.  The updater normally refuses
    ...
    // `clawcodex update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: CLAWCODEX_CONFIG_DIR,
      env: {
        ...process.env,
        CLAWCODEX_CONFIG_DIR,
        PATH: pathWithClawCodexManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another ClawCodex update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(CLAWCODEX_CONFIG_DIR, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvClawCodex = path.join(venvBin, IS_WINDOWS ? 'clawcodex.exe' : 'clawcodex')
  ...
  // not just the `clawcodex.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvClawCodex) || fileExists(path.join(updateRoot, '.clawcodex-bootstrap-complete'))
...
// Resolve the clawcodex CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `clawcodex` on PATH.
function resolveClawCodexCliBinary(updateRoot) {
  const venvClawCodex = path.join(updateRoot, 'venv', 'bin', 'clawcodex')

  if (fileExists(venvClawCodex)) {
    return venvClawCodex
  }

  return findOnPath('clawcodex') || null
}
...
// Python-level pre-update snapshot inside `clawcodex update`.
function preflightStateDb(clawcodexHome, rememberLog) {
  const stateDbPath = path.join(clawcodexHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(clawcodexHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(clawcodexHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(clawcodexHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`clawcodex update`) + OS-aware GUI rebuild
// (`clawcodex desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const clawcodex = resolveClawCodexCliBinary(updateRoot)

  if (!clawcodex) {
    emitUpdateProgress({ stage: 'manual', message: 'clawcodex update', percent: null })

    return { ok: true, manual: true, command: 'clawcodex update', clawcodexRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(CLAWCODEX_CONFIG_DIR, rememberLog)

  // Put the ClawCodex-managed Node and the venv on PATH so `clawcodex desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\clawcodex\\node, not node\\bin.
  // PYTHONUNBUFFERED: `clawcodex update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    CLAWCODEX_CONFIG_DIR,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithClawCodexManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `clawcodex update` reaps stale `clawcodex serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads CLAWCODEX_DESKTOP_CHILD_PID and excludes
  ...
  if (clawcodexProcess && Number.isInteger(clawcodexProcess.pid)) {
    desktopChildPids.push(clawcodexProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.CLAWCODEX_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating ClawCodex (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(clawcodex, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'clawcodex update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'clawcodex update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(clawcodex, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart ClawCodex to retry.',
  ...
  // Linux in-app update terminal state (#45205). `clawcodex desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      ...
      const scriptPath = path.join(app.getPath('temp'), `clawcodex-desktop-update-${Date.now()}.sh`)
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the ClawCodex desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen ClawCodex to finish.'
    }
  }
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart ClawCodex to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.clawcodex-update-new"; then
    rm -rf "$DST.clawcodex-update-old" 2>/dev/null || true
    mv "$DST" "$DST.clawcodex-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.clawcodex-update-new" "$DST"
    rm -rf "$DST.clawcodex-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart ClawCodex to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_CLAWCODEX_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isClawCodexSourceRoot(ACTIVE_CLAWCODEX_ROOT) &&
    fileExists(venvPython) &&
    canImportClawCodexCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CLAWCODEX_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `clawcodex
...
function resolveWebDist() {
  const override = process.env.CLAWCODEX_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CLAWCODEX_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: clawcodex desktop --force-build`
  )
...
function resolveClawCodexCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/ClawCodex.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.CLAWCODEX_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveClawCodexCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_CLAWCODEX_ROOT, the
...
function resolveClawCodexBackend(backendArgs) {
  // 1. Explicit override -- CLAWCODEX_DESKTOP_CLAWCODEX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLAWCODEX_DESKTOP_CLAWCODEX_ROOT && path.resolve(process.env.CLAWCODEX_DESKTOP_CLAWCODEX_ROOT)

  if (overrideRoot && isClawCodexSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `ClawCodex source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `clawcodex` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isClawCodexSourceRoot.)
  if (!IS_PACKAGED && isClawCodexSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `ClawCodex source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_CLAWCODEX_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\clawcodex\\clawcodex (Windows) or ~/.clawcodex/clawcodex.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active ClawCodex runtime at ${ACTIVE_CLAWCODEX_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `clawcodex` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLAWCODEX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLAWCODEX_DESKTOP_IGNORE_EXISTING !== '1') {
    let clawcodexCommand = null
    const clawcodexOverride = process.env.CLAWCODEX_DESKTOP_CLAWCODEX

    if (clawcodexOverride) {
      const resolvedOverride = findOnPath(clawcodexOverride)

      if (resolvedOverride) {
        clawcodexCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(clawcodexOverride, { isWsl: IS_WSL })) {
        clawcodexCommand = clawcodexOverride
      } else {
        rememberLog(`Ignoring Windows ClawCodex override under WSL: ${clawcodexOverride}`)
      }
    } else {
      clawcodexCommand = findOnPath('clawcodex')
    }

    if (clawcodexCommand) {
      if (looksLikeDesktopAppBinary(clawcodexCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving ClawCodex CLI: ${clawcodexCommand}`)
        clawcodexCommand = null
      }
    }

    if (clawcodexCommand) {
      const unwrapped = unwrapWindowsVenvClawCodexCommand(clawcodexCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `clawcodex` shim
      ...
      const shellForProbe = isCommandScript(clawcodexCommand)

      // CLAWCODEX_DESKTOP_CLAWCODEX is an explicit deployment override (used by
      ...
      if (shouldTrustClawCodexOverride(clawcodexOverride) || verifyClawCodexCli(clawcodexCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing ClawCodex CLI at ${clawcodexCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clawcodex_cli module via system Python.
  ...
  if (python) {
    ...
    // having clawcodex_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clawcodex\clawcodex\venv.
    if (canImportClawCodexCli(python)) {
      return {
        kind: 'python',
    ...
    rememberLog(`Ignoring system Python ${python}: clawcodex_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveClawCodexBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'ClawCodex not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_CLAWCODEX_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveClawCodexBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no ClawCodex install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'ClawCodex recovery was handed off to ClawCodex Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      clawcodexHome: CLAWCODEX_CONFIG_DIR,
      logRoot: path.join(CLAWCODEX_CONFIG_DIR, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('ClawCodex install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `ClawCodex bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLAWCODEX_CONFIG_DIR, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClawCodex() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clawcodex:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveClawCodexBackend(backend.args))
  }
  ...
  if (!isClawCodexSourceRoot(ACTIVE_CLAWCODEX_ROOT)) {
    throw new Error(
      `ClawCodex install at ${ACTIVE_CLAWCODEX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. ClawCodex' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clawcodex\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clawcodex` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for ClawCodex on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch ClawCodex.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isClawCodexSourceRoot()
    // plus an importable clawcodex_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `ClawCodex venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `ClawCodex at ${ACTIVE_CLAWCODEX_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'ClawCodex runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----clawcodex-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported ClawCodex backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-ClawCodex-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the ClawCodex backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to ClawCodex backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-ClawCodex-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clawcodex:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveClawCodexCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('clawcodex:preview-file-changed', payload)
}
...
// waitForClawCodexReady how to read a 401 — rejected session vs gated route.
...
async function waitForClawCodex(baseUrl, token, signal?, authMode?) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForClawCodexReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('clawcodex:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('clawcodex:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('clawcodex:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting ClawCodex...".
function sendPowerResume() {
  ...
  webContents.send('clawcodex:power-resume')
}
...
// 'clawcodex:power-battery' push below.
ipcMain.handle('clawcodex:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('clawcodex:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('clawcodex:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('clawcodex:window-state-changed', state)
}
...
//
// Hosted ClawCodex gateways gate the dashboard behind an OAuth provider (e.g.
// ClawCodex contributors) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``clawcodex_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / clawcodex #37247).
//     Both are set as HttpOnly cookies (``clawcodex_session_at`` ~15 min,
//     ``clawcodex_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:clawcodex-remote-oauth'
...
// "not signed in", which on the initial boot path (startClawCodex → the renderer's
// single-shot boot() with no retry) surfaced as the "ClawCodex couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "ClawCodex
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to ClawCodex Cloud agent…' : 'Sign in to ClawCodex gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to ClawCodex backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in clawcodex_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- ClawCodex Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:clawcodex-remote-oauth`:
...
// convention (clawcodex_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every ClawCodex surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.invalid'

function resolvePortalBaseUrl() {
  const raw = process.env.CLAWCODEX_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the ClawCodex gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for clawcodex_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a ClawCodex Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to ClawCodex Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (ClawCodex Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to ClawCodex Cloud. Open Settings → Gateway, choose ClawCodex Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your ClawCodex Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your ClawCodex Cloud session has expired. Sign in to ClawCodex Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the ClawCodex Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root CLAWCODEX_CONFIG_DIR explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CLAWCODEX_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.CLAWCODEX_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted ClawCodex Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteClawCodexPath: (ssh || savedSsh)?.remoteClawCodexPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by CLAWCODEX_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the ClawCodex Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered ClawCodex Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteClawCodexPath: input.sshRemoteClawCodexPath ?? existingBlock.remoteClawCodexPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (clawcodex #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote ClawCodex gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote ClawCodex gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote ClawCodex gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.CLAWCODEX_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteClawCodexPath || '')
    ...
    result = await lifecycle({
      ...
      remoteClawCodexPath: sshConfig.remoteClawCodexPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForClawCodex: (baseUrl, token) => waitForClawCodex(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    clawcodexVersion: result.clawcodexVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.clawcodexVersion || 'clawcodex (version unknown)'} at ${result.clawcodexPath || '?'}`
  )
  ...
  return { ...connection, remoteClawCodexVersion: result.clawcodexVersion || '' }
}
...
//   2. env override (CLAWCODEX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.CLAWCODEX_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.CLAWCODEX_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'CLAWCODEX_DESKTOP_REMOTE_URL is set but CLAWCODEX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote ClawCodex backend.'
      )
    }
...
// env → global) so a startClawCodex() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every ClawCodex
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with ClawCodex contributors" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteClawCodexPath: input.sshRemoteClawCodexPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteClawCodexPath || '')
          let clawcodexPath
          let clawcodexVersion
          let supported

          if (platform.os === 'Windows') {
            const runtime = platform
            clawcodexPath = runtime.clawcodexPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.clawcodexPath])
            clawcodexVersion = inspection.version
            supported = inspection.supported
          } else {
            clawcodexPath = await remoteLifecycle.locateClawCodex(ssh, sshConfig.remoteClawCodexPath || '')
            clawcodexVersion = await remoteLifecycle.probeClawCodexVersion(ssh, clawcodexPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, clawcodexPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update ClawCodex on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteClawCodexPath: clawcodexPath,
            remoteClawCodexVersion: clawcodexVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClawCodex())
  ...
  // connect to ClawCodex gateway". Mirror the renderer's connect here so the test
...
function resetClawCodexConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const clawcodexProcess = backendConnectionState.invalidate()
  stopBackendChild(clawcodexProcess)
...
// startClawCodex() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetClawCodexConnection() invalidates it.
  const clawcodexProcess = backendConnectionState.getProcess()
  const dying = clawcodexProcess && !clawcodexProcess.killed ? clawcodexProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('clawcodex:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startClawCodex()
...
// local-spawn portion of startClawCodex() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForClawCodex(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited CLAWCODEX_CONFIG_DIR env (see _apply_profile_override
  // step 3 in clawcodex_cli/main.py), so the child re-homes to this profile.
  ...
  rememberLog(`Starting ClawCodex backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: clawcodexCwd,
        CLAWCODEX_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        CLAWCODEX_DESKTOP: '1',
        CLAWCODEX_WEB_DIST: webDist,
        ...(readyFile ? { CLAWCODEX_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`ClawCodex backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`ClawCodex backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`ClawCodex backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForClawCodex(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `ClawCodex backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `ClawCodex backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_clawcodex_home() recreates the deleted profile directory.
//
...
async function startClawCodex() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClawCodex() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving ClawCodex backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote ClawCodex backend at ${remote.baseUrl}`, 24)
      await waitForClawCodex(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote ClawCodex backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteClawCodexVersion: remote.remoteClawCodexVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: clawcodexLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.clawcodex/active_profile file) and
    // resolves CLAWCODEX_CONFIG_DIR the same way `clawcodex -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving ClawCodex runtime', 28)

        return resolveClawCodexBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting ClawCodex backend via ${backend.label}`, 84)
    rememberLog(`Starting ClawCodex backend via ${backend.label}`)

    const clawcodexProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: clawcodexCwd,
        env: {
          ...process.env,
          // Explicitly pin CLAWCODEX_CONFIG_DIR for the child so Python's get_clawcodex_home()
          // resolves to the SAME location our resolveClawCodexHome() picked. Without
          // this pin, Python falls back to ~/.clawcodex on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\clawcodex, which differs from C:\Users\<u>\.clawcodex.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CLAWCODEX_CONFIG_DIR via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, clawcodexProcess)

    if (!processOwner) {
      stopBackendChild(clawcodexProcess)
      throw new Error('ClawCodex backend start was superseded by a newer connection attempt.')
    }

    clawcodexProcess.stdout.on('data', rememberLog)
    clawcodexProcess.stderr.on('data', rememberLog)
    ...
    clawcodexProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ClawCodex backend error: ${error.message}`)
        rejectBackendStart?.(new Error('ClawCodex backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`ClawCodex backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `ClawCodex backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    clawcodexProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ClawCodex backend exit (${signal || code})`)
      ...
      rememberLog(`ClawCodex backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `ClawCodex backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `ClawCodex backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentClawCodexLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for ClawCodex backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(clawcodexProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for ClawCodex backend to become ready', 90)
    await Promise.race([waitForClawCodex(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => clawcodexProcess.exitCode === null && !clawcodexProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local ClawCodex backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'ClawCodex backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while ClawCodex is
...
// pushes pet state over IPC (clawcodex:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via clawcodex:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main ClawCodex app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the ClawCodex icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see clawcodex:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep ClawCodex in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('clawcodex:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('clawcodex:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('clawcodex:quick-entry:shown')
}
...
function createWindow() {
  ...
  startClawCodex().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('clawcodex:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting ClawCodex…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('clawcodex:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetClawCodexConnection,
...
ipcMain.handle('clawcodex:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('clawcodex:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('clawcodex:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('clawcodex:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('clawcodex:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('clawcodex:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('clawcodex:zoom:get', event => {
...
ipcMain.on('clawcodex:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clawcodex:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('clawcodex:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('clawcodex:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('clawcodex:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('clawcodex:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('clawcodex:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('clawcodex:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('clawcodex:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('clawcodex:pet-overlay:control', payload)
})
ipcMain.handle('clawcodex:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startClawCodex() call restarts the
...
ipcMain.handle('clawcodex:bootstrap:repair', async () => {
  // Forceful repair: force the next startClawCodex() through the full installer
  ...
  // "bypass active runtime" path inside startClawCodex() and fall through
  ...
  resetClawCodexConnection()

  return { ok: true }
})
ipcMain.handle('clawcodex:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('clawcodex:bootstrap:cancel', async () => {
...
ipcMain.handle('clawcodex:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clawcodex:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clawcodex:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clawcodex:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('clawcodex:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('clawcodex:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('clawcodex:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clawcodex:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startClawCodex() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('clawcodex:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- ClawCodex Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('clawcodex:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('clawcodex:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('clawcodex:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('clawcodex:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('clawcodex:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLAWCODEX_CONFIG_DIR. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clawcodex:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clawcodex:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clawcodex:api', async (_event, request) => {
  ...
  // backend calls ensure_clawcodex_home() which recreates the profile directory,
...
ipcMain.handle('clawcodex:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('clawcodex:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('clawcodex:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('clawcodex:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('clawcodex:data-url-read-max:get', () => ({
...
ipcMain.handle('clawcodex:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('clawcodex:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clawcodex:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('clawcodex:readFileText', async (_event, filePath) => {
...
ipcMain.handle('clawcodex:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('clawcodex:writeClipboard', (_event, text) => {
...
ipcMain.handle('clawcodex:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('clawcodex:readClipboard', () => clipboard.readText())

ipcMain.handle('clawcodex:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('clawcodex:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clawcodex:saveClipboardImage', async () => {
...
ipcMain.handle('clawcodex:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clawcodex:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clawcodex:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('clawcodex:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('clawcodex:active-work', (event, payload) => {
...
ipcMain.on('clawcodex:titlebar-theme', (_event, payload) => {
...
ipcMain.on('clawcodex:native-theme', (_event, mode) => {
...
ipcMain.on('clawcodex:translucency', (_event, payload) => {
...
ipcMain.on('clawcodex:keep-awake', (_event, on) => {
...
ipcMain.handle('clawcodex:quick-entry:settings:get', async () => {
...
ipcMain.handle('clawcodex:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('clawcodex:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('clawcodex:quick-entry:submit', {
...
ipcMain.on('clawcodex:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('clawcodex:quick-entry:state', payload)
  }
})

ipcMain.on('clawcodex:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('clawcodex:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `clawcodex:window:openSession`).
...
ipcMain.handle('clawcodex:find-in-page', (event, query, options) => {
...
ipcMain.handle('clawcodex:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clawcodex:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveClawCodexCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clawcodex:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveClawCodexCwd()
}))

ipcMain.handle('clawcodex:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('clawcodex:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clawcodex:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clawcodex:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clawcodex:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('clawcodex:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: clawcodexLog.slice(-200) }))
...
function terminalShellCommand() {
  // CLAWCODEX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.CLAWCODEX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip ClawCodex' TUI into
  ...
  env.TERM_PROGRAM = 'ClawCodex'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a clawcodex/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from CLAWCODEX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.CLAWCODEX_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `clawcodex:terminal:${id}:${suffix}`
}
...
ipcMain.handle('clawcodex:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('clawcodex:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('clawcodex:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('clawcodex:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<CLAWCODEX_CONFIG_DIR>/desktop-plugins`,
// resolved from the main-process CLAWCODEX_CONFIG_DIR (see resolveClawCodexHome) — NOT from
// the connected backend. A remote backend reports its own `clawcodex_home` over
...
ipcMain.handle('clawcodex:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped clawcodex_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(CLAWCODEX_CONFIG_DIR, 'profiles', profile) : CLAWCODEX_CONFIG_DIR
...
ipcMain.handle('clawcodex:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('clawcodex:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('clawcodex:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('clawcodex:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('clawcodex:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('clawcodex:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('clawcodex:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('clawcodex:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('clawcodex:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('clawcodex:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('clawcodex:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('clawcodex:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('clawcodex:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('clawcodex:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('clawcodex:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('clawcodex:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('clawcodex:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('clawcodex:terminal:write', (_event, id, data) => {
...
ipcMain.handle('clawcodex:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clawcodex:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clawcodex:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('clawcodex:updates:check', async () =>
...
ipcMain.handle('clawcodex:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clawcodex:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clawcodex:updates:branch:set', async (_event, name) => {
...
// Re-resolve the live ClawCodex version and push it into the native About panel
// just before showing it, so an in-place `clawcodex update` is reflected without
...
ipcMain.handle('clawcodex:version', async () => ({
  appVersion: resolveClawCodexVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  clawcodexRoot: resolveUpdateRoot()
}))
...
// the actual removal via `clawcodex uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (clawcodex_cli/uninstall.py + clawcodex_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CLAWCODEX_ROOT
  ...
  const fallback = () => ({
    clawcodex_home: CLAWCODEX_CONFIG_DIR,
    agent_installed: isClawCodexSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no ClawCodex agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import clawcodex_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_CLAWCODEX_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_CLAWCODEX_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_CLAWCODEX_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `clawcodex-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `clawcodex-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('clawcodex:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('clawcodex:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('clawcodex:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('clawcodex:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// clawcodex:// deep links (e.g. clawcodex://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const CLAWCODEX_PROTOCOL = 'clawcodex'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${CLAWCODEX_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // clawcodex://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('clawcodex:deep-link', payload)
...
ipcMain.handle('clawcodex:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CLAWCODEX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CLAWCODEX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(CLAWCODEX_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `clawcodex://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching clawcodex:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting ClawCodex never keeps another app's chord hostage.
