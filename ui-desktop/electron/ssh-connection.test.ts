...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env CLAWCODEX_DASHBOARD_SESSION_TOKEN=abc123deadbeef CLAWCODEX_DESKTOP=1 clawcodex dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /CLAWCODEX_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /CLAWCODEX_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-ClawCodex-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-ClawCodex-Session-Token: hdr_888'), /X-ClawCodex-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-ClawCodex-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath is stable, short, and host-distinct', () => {
  ...
  // ([\\/]: path.join emits backslashes on Windows)
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('clawcodex', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v clawcodex', 15000)
  assert.equal(args[args.length - 1], 'command -v clawcodex')
...
test('open() establishes the master when not already alive', async () => {
  ...
  // mux: true — the Windows constructor defaults to no-mux (Win32 OpenSSH never
  // implemented ControlMaster); these lifecycle tests exercise the mux
  // machinery itself, so pin it on for every platform.
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `clawcodex-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `clawcodex-ssh-test-${process.pid}-${Date.now()}`)
...
test('open() rejects a control-dir that is a symlink', async () => {
  if (process.platform === 'win32') {
    // open()'s symlink/ownership control-dir checks are POSIX-only in the
    // source (and symlinkSync needs elevation on Windows).
    return
  }
