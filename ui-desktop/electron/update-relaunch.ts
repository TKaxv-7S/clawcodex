 ...
 * Background
 * ----------
 * After `clawcodex update` + `clawcodex desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything CLAWCODEX_DESKTOP_* is preserved
// plus CLAWCODEX_CONFIG_DIR. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['CLAWCODEX_CONFIG_DIR', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['CLAWCODEX_DESKTOP_']
